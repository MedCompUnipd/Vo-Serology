...
# May 2020 ; (ii) over all positive subjects in May 2020 and with decaying     #
# antibodies between May 2020 and November 2020 and between November 2020 and  #
# June 2021; (iii) over all positive subjects in May 2020 and with             #
# antibodies not doubling between May 2020 and November 2020 and between       #
# November 2020 and June 2021.                                                 # 
################################################################################
compute_half_life <- function(ds, type){
  
  days <- as.numeric(as.Date("06/05/2020", "%m/%d/%Y") - 
                       as.Date("05/01/2020", "%m/%d/%Y"))

  if(type == "Abbott"){
    
    # positive in May 
    mean_p <- mean(ds$hl_Abbott[which(ds$Abbott_qualitative_May_2020 == "Positive")], 
                   na.rm = TRUE)
    median_p <- median(ds$hl_Abbott[which(ds$Abbott_qualitative_May_2020 == "Positive")], 
                       na.rm = TRUE)
    
    # positive in May and decreasing 
    mean_pn <- mean(ds$hl_Abbott[which(ds$Abbott_qualitative_May_2020 == "Positive" & 
                              ds$slope_Abbott <= 0)], na.rm = TRUE)
    median_pn <- median(ds$hl_Abbott[which(ds$Abbott_qualitative_May_2020 == "Positive"& 
                                  ds$slope_Abbott <= 0)], na.rm = TRUE)
    
    # positive in May and not doubling 
    mean_pd <- mean(ds$hl_Abbott[which(ds$Abbott_qualitative_May_2020 == "Positive" & 
                              ds$slope_Abbott <= log(2)/days)], na.rm = TRUE)
    median_pd <- median(ds$hl_Abbott[which(ds$Abbott_qualitative_May_2020 == "Positive"& 
                                  ds$slope_Abbott <= log(2)/days)], na.rm = TRUE)
    
  }else if(type == "DiaSorin"){
    
    # positive in May 
    mean_p <- mean(ds$hl_Diasorin[which(ds$DiaSorin_S1_S2_qualitative_May_2020 == "Positive" & 
                                          is.infinite(ds$hl_Diasorin) == FALSE) ], 
                   na.rm = TRUE)
    median_p <- median(ds$hl_Diasorin[which(ds$DiaSorin_S1_S2_qualitative_May_2020 == "Positive"& 
                                              is.infinite(ds$hl_Diasorin) == FALSE)], 
                       na.rm = TRUE)
    
    # positive in May and decreasing 
    mean_pn <- mean(ds$hl_Diasorin[which(ds$DiaSorin_S1_S2_qualitative_May_2020 == "Positive" & 
                              ds$slope_Diasorin <= 0 & 
                                is.infinite(ds$hl_Diasorin) == FALSE)], na.rm = TRUE)
    median_pn <- median(ds$hl_Diasorin[which(ds$DiaSorin_S1_S2_qualitative_May_2020 == "Positive"& 
                                  ds$slope_Diasorin <= 0 & 
                                    is.infinite(ds$hl_Diasorin) == FALSE)], na.rm = TRUE)
    
    # positive in May and not doubling 
    mean_pd <- mean(ds$hl_Diasorin[which(ds$DiaSorin_S1_S2_qualitative_May_2020 == "Positive" & 
                              ds$slope_Diasorin <= log(2)/days & 
                                is.infinite(ds$hl_Diasorin) == FALSE)], na.rm = TRUE)
    median_pd <- median(ds$hl_Diasorin[which(ds$DiaSorin_S1_S2_qualitative_May_2020 == "Positive"& 
                                  ds$slope_Diasorin <= log(2)/days & 
                                    is.infinite(ds$hl_Diasorin) == FALSE)], na.rm = TRUE)
    ...
    # positive in May 
    mean_p <- mean(ds$hl_Roche[which(ds$Roche_qualitative_May_2020 == "Positive")], 
                   na.rm = TRUE)
    median_p <- median(ds$hl_Roche[which(ds$Roche_qualitative_May_2020 == "Positive")], 
                       na.rm = TRUE)
    
    # positive in May and decreasing 
    mean_pn <- mean(ds$hl_Roche[which(ds$Roche_qualitative_May_2020 == "Positive" & 
                                           ds$slope_Roche <= 0)], na.rm = TRUE)
    median_pn <- median(ds$hl_Roche[which(ds$Roche_qualitative_May_2020 == "Positive"& 
                                               ds$slope_Roche <= 0)], na.rm = TRUE)
    
    # positive in May and not doubling 
    mean_pd <- mean(ds$hl_Roche[which(ds$Roche_qualitative_May_2020 == "Positive" & 
                                           ds$slope_Roche <= log(2)/days)], na.rm = TRUE)
    median_pd <- median(ds$hl_Roche[which(ds$Roche_qualitative_May_2020 == "Positive"& 
                                               ds$slope_Roche <= log(2)/days)], na.rm = TRUE)
